...
export async function POST(req: NextRequest) {
  try {
    ...
    const thresholdBps = Number(body?.thresholdBps ?? 100); // 1%
    const slippageBps = Number(body?.slippageBps ?? 50); // 0.5%
    const boundsBps = Number(body?.boundsBps ?? 50); // ±0.5%
    ...
    // Pyth fee
    const feeWei = await client.readContract({ address: PYTH_EVM as Address, abi: pythAbi, functionName: "getUpdateFee", args: [updateData] });
